...
- (void)_initializeStreams;
{
    ...
    if (_secure) {
        ...
        if (self.allowUntrustedConnections) {
            [SSLOptions setValue:[NSNumber numberWithBool:NO] forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
            NSLog(@"SocketRocket: allowUntrustedConnections enabled, allowing connection to any root cert");
        }
...
- (void)main;
{
    @autoreleasepool {
        ...
        NSTimer *timer = [[NSTimer alloc] initWithFireDate:[NSDate distantFuture] interval:0.0 target:self selector:@selector(neverFire) userInfo:nil repeats:NO];
